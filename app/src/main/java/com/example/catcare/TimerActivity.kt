...
class TimerActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnStart.setOnClickListener {
            ...
            if (h == 0 && m == 0) {
                Toast.makeText(this, "ต้องตั้งเวลาอย่างน้อย 1 นาที", Toast.LENGTH_SHORT).show()
                return@setOnClickListener
            }
            Prefs.setFeedInterval(this, h, m)
            AlarmHelper.scheduleNextFeeding(applicationContext)
            tvStatus.text = "เริ่มแจ้งเตือนทุก ๆ ${h} ชม. ${m} นาที"
        }

        btnStop.setOnClickListener {
            AlarmHelper.cancelFeeding(applicationContext)
